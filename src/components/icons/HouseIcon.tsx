"use client";
import * as React from "react";
import { createSvgIcon } from "@mui/material/utils";
import type { SvgIconProps } from "@mui/material/SvgIcon";

const HouseIconBase = createSvgIcon(
  <svg
    xmlns={"http://www.w3.org/2000/svg"}
    width={"24"}
    height={"24"}
    viewBox={"0 0 24 24"}
    fill={"none"}
  >
    <path
      d={
        "M15 21.9995V13.9995C15 13.7343 14.8946 13.4799 14.7071 13.2924C14.5196 13.1049 14.2652 12.9995 14 12.9995H10C9.73478 12.9995 9.48043 13.1049 9.29289 13.2924C9.10536 13.4799 9 13.7343 9 13.9995V21.9995"
      }
      stroke={"currentColor"}
      strokeLinecap={"round"}
      strokeLinejoin={"round"}
    />
    <path
      d={
        "M3 10.9995C2.99993 10.7085 3.06333 10.4211 3.18579 10.1572C3.30824 9.89329 3.4868 9.65928 3.709 9.47148L10.709 3.47248C11.07 3.16739 11.5274 3 12 3C12.4726 3 12.93 3.16739 13.291 3.47248L20.291 9.47148C20.5132 9.65928 20.6918 9.89329 20.8142 10.1572C20.9367 10.4211 21.0001 10.7085 21 10.9995V19.9995C21 20.5299 20.7893 21.0386 20.4142 21.4137C20.0391 21.7888 19.5304 21.9995 19 21.9995H5C4.46957 21.9995 3.96086 21.7888 3.58579 21.4137C3.21071 21.0386 3 20.5299 3 19.9995V10.9995Z"
      }
      stroke={"currentColor"}
      strokeLinecap={"round"}
      strokeLinejoin={"round"}
    />
  </svg>,
  "HouseIconBase",
);

export type HouseIconProps = SvgIconProps & {
  strokeWidth?: number | string;
};

const HouseIcon = React.forwardRef<SVGSVGElement, HouseIconProps>(
  ({ strokeWidth = 1.5, sx, ...rest }, ref) => (
    <HouseIconBase
      ref={ref}
      sx={{ ...sx, strokeWidth, "& *": { vectorEffect: "non-scaling-stroke" } }}
      {...rest}
    />
  ),
);

HouseIcon.displayName = "HouseIcon";

export default HouseIcon;
